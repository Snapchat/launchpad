...
## MPC
```bash
docker build -t launchpad-cloud-shell --build-arg LAUNCHPAD_MODE=mpc --build-arg .
...
## Relay
```bash
docker build -t launchpad-cloud-shell --build-arg LAUNCHPAD_MODE=relay --build-arg .
