...
## MPC
```bash
docker build -t launchpad-cloud-shell --build-arg LAUNCHPAD_MODE=mpc --build-arg VERSION=latest .
...
## Relay
```bash
docker build -t launchpad-cloud-shell --build-arg LAUNCHPAD_MODE=relay --build-arg VERSION=latest .
